...
void posordem (node *n){
  if (!n->l && !n->r) {   //é folha??
	...
	if(n->l) posordem(n->l);  //tem filho a esqueda?
	if(n->r) posordem(n->r);  // tem filho a direita??
	if(n->l && n->r) {
		if(n->l->lvl > n->r->lvl) n->lvl = n->l->lvl+1;
		else  n->lvl = n->r->lvl+1;
	}
	else{
		if(n->l) n->lvl = n->l->lvl + 1;
		else n->lvl = n->r->lvl + 1;
	}
}
...
int getLevelUtil(struct node *node, int data, int level)
{
	...
	int downlevel = getLevelUtil(node->l, data, level-1);
	if (downlevel != 0)
        return downlevel;
	node-> lvl = downlevel;
	return downlevel;

	downlevel = getLevelUtil(node->r, data, level-1);
	return downlevel;
}
...
int main(){
	...
	// INSERTION TEST
  for (size_t i = 1; i < 6; i++) insert(&root, new(i));
	//while(c++ < TEST_NUMBER
	//for (size_t i = 0; i < 5; i++) insert(&root, new(rand() % 150));
	printf("root: %d\n", root->value);
	printf("\n > LVL -> ");
  posordem(root);
	printf("\n");
